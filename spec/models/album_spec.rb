  ...
  context 'before_destroy' do
    ...
    context 'with artists, genres and styles that do not have other albums' do
      ...
      it 'deletes the album, artists, genres and styles' do
        expect { call }.to change(Album, :count)
          .by(-1).and change(Artist, :count)
          .by(-album.artists.length).and change(Genre, :count)
          .by(-album.genres.length).and change(Style, :count)
          .by(-album.styles.length)
      end
    end
